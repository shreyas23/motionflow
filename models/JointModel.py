    ...
    def run_pwc(self, input_dict, x1_raw, x2_raw, k1, k2):
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid[::-1], x2_pyramid[::-1])):
            ...
            if l == 0:
                ...
                if self.args.use_bottleneck:
                    x1_out, flow_f_res, disp_l1_res, mask_l1, pose_f_res, pose_f_out = self.flow_estimators[l](torch.cat([
        ...
        output_dict['disps_l2'] = upsample_outputs_as(disps_2[::-1], x1_pyramid)
        output_dict['masks_l1'] = upsample_outputs_as(masks_1[::-1], x1_pyramid)
    ...
    def forward(self, input_dict):
        ...
        if self.args.evaluation or (not self.training):
            ...
            for ii in range(0, len(output_dict_flip['flows_f'])):
                ...
                flow_f_pp, rigidity_mask_l1 = pose_process_flow(img_l1, img_l2, output_dict['pose_f'][ii], flow_f_pp, disps_l1_pp[ii], masks_l1_pp[ii], K1, aug_size, self.args.mask_thresh, self.args.flow_diff_thresh)
                flow_b_pp, rigidity_mask_l2 = pose_process_flow(img_l2, img_l1, output_dict['pose_b'][ii], flow_b_pp, disps_l2_pp[ii], masks_l2_pp[ii], K2, aug_size, self.args.mask_thresh, self.args.flow_diff_thresh)

...
class SceneNetStereoJoint(nn.Module):
    def __init__(self, args):
        ...
        for l, ch in enumerate(self.num_chs[::-1]):
            if l > self.output_level:
                break
            if l == 0:
                num_ch_in = self.dim_corr + ch + self.disp_range + ch
            else:
                num_ch_in = self.dim_corr + ch + self.disp_range + ch + 32 + 3 + 1 + 6 + 1
    ...
    def run_pwc(self, input_dict, x1_raw, x2_raw, r1_raw, r2_raw, k1, k2):
        ...
        for l, (x1, x2, r1, r2) in enumerate(zip(x1_pyramid, x2_pyramid, r1_pyramid, r2_pyramid)):
            ...
            # monosf estimator
            if l == 0:
                x1_out, flow_f, disp_l1, mask_l1, pose_f, pose_f_out = self.flow_estimators[l](torch.cat([out_corr_relu_f, x2, disp_corr1, x1], dim=1))
                x2_out, flow_b, disp_l2, mask_l2, pose_b, pose_b_out = self.flow_estimators[l](torch.cat([out_corr_relu_b, x1, disp_corr2, x2], dim=1))
            else:
                x1_out, flow_f_res, disp_l1, mask_l1, pose_f, pose_f_out = self.flow_estimators[l](torch.cat([
                    out_corr_relu_f, x2, disp_corr1, x1, x1_out, flow_f, disp_l1, mask_l1, pose_f_out], dim=1))
                x2_out, flow_b_res, disp_l2, mask_l2, pose_b, pose_b_out = self.flow_estimators[l](torch.cat([
                    out_corr_relu_b, x1, disp_corr2, x2, x2_out, flow_b, disp_l2, mask_l2, pose_b_out], dim=1))
